...
int Camera::get_pic(cv::Mat *srcimg, GlobalParam &gp)
{
    ...
    *srcimg = cv::Mat(stOutFrame.stFrameInfo.nHeight, stOutFrame.stFrameInfo.nWidth, CV_8UC3, pDataForRGB);
    if (srcimg->empty() == 1)
        return -1;
    // 将srcimg由RGB转化为BGR图片
    cv::cvtColor(*srcimg, *srcimg, cv::COLOR_RGB2BGR); // COLOR_RGB2BGR  COLOR_BayerRG2BGR COLOR_BayerRG2BGR
...
int Camera::set_param_once(GlobalParam &gp)
{
    ...
    this->nRet = MV_CC_SetTriggerMode(this->handle, MV_TRIGGER_MODE_ON);
#else
